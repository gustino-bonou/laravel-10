<x-mail::message>
# @if ($task->parseDateInCarbon($task->finish_at)->isSameDay(now()) && $task->finished_at === null)
Cette tache devrait terminer aujourd'hui <br><br>
<a href="{{ route('task.marque.finish', ['task' => $task->id])}}">Marquer comme terminée ?</a> <br><br>
...
<a href="{{ route('task.marque.begin', ['task' => $task->id])}}">  Marquer comme démarrée ? </a> 
@endif
