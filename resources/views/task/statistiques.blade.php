



    <table class="table">
        <thead>
          <tr class="table-entete">
            <th scope="col" class="table-entete">Stats</th>
            <th scope="col"><a href="">Nrb Total Taches</a></th>
            <th scope="col">Taches terminées</th>
            <th scope="col">Taches terminées en retard</th>
            <th scope="col">Taches en cours</th>
            <th scope="col">Taches démarrée avec retard</th>
            <th scope="col">Taches non démarrée</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <th scope="row" class="table-entete">Nombre</th>
            <td>{{ $nbrTotalTaches }}</td>
            <td>{{ $nbrTachesTerminees }}</td>
            <td>{{ $nbrTachesTermineesEnRetard }}</td>
            <td>{{ $nbrTachesEnCours }}</td>
            <td>{{ $nbrTachesDemarreesEnRetard }}</td>
            <td>{{ $nrbTachesNondemarrees }}</td>
            
          </tr>

        </tbody>
      </table>

      
    